from keys import weather_key, lat, lon, state, city
...
class Weather():
    ...
    def get_data(self, key, lat, lon):
        resp = requests.get(f"http://api.openweathermap.org/data/2.5/forecast?lat={lat}&lon={lon}&units=metric&appid={key}")
